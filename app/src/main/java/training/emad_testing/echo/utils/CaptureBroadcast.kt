package training.emad_testing.echo.utils
        ...
        if (intent?.action == Intent.ACTION_NEW_OUTGOING_CALL) {
            ...
            try {
                if (SongPlayingFragment.SongPlayingStatified.mediaplayer.isPlaying) {
                    ...
                    SongPlayingFragment.SongPlayingStatified.playpauseImageButton?.setBackgroundResource(R.drawable.play_icon)
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
        } else {
            val em: TelephonyManager = context?.getSystemService(Service.TELEPHONY_SERVICE) as TelephonyManager
            when (em.callState) {
